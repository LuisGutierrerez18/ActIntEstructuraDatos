...
void heapifyDown(DLLNode<T> *node) {
  while (node->next != nullptr && node->data.getIpTotal() < node->next->data.getIpTotal()) {
...
public:
  ...
  std::vector<int> getIpAdint(int index);
  long long int getIpTotal(int index);
};
...
// Complejidad O(n)
template <class T> std::vector<int> DLinkedList<T>::getIpAdint(int index) {
  if (index < 0 || index >= numElements || head == nullptr) {
    // Manejar caso de índice fuera de rango o lista vacía
    throw std::out_of_range("Índice fuera de rango o lista vacía");
  }
  ...
  return current->data.getIpAdInt();
}

template <class T> long long int DLinkedList<T>::getIpTotal(int index) {
  if (index < 0 || index >= numElements || head == nullptr) {
    // Manejar caso de índice fuera de rango o lista vacía
    throw std::out_of_range("Índice fuera de rango o lista vacía");
  }
  ...
  return current->data.getIpTotal();
}
