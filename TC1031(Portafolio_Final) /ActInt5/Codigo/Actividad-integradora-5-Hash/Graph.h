...
public:
  ...
  void calculateInDegrees(const std::string &filename);
  void calculateDegrees(const std::string &filename);
  void displayDegrees(const std::string &filename);
  ...
  long long int stringIpToInt(const std::string &line);
};
...
template <class T>
long long int Graph<T>::stringIpToInt(const std::string &line) {
  int pos3 = pow(256, 3);
  int pos2 = pow(256, 2);
  int pos1 = pow(256, 1);
  std::regex ipRegex(
...
void Graph<T>::readGraph(std::istream &input, int _directed, int _weighted) {
  ...
  for (int idx = 0; idx < decimalIPs.size(); ++idx) {
    ...
    getNodeIdentifier(decimalIPs[idx].first);
...
//O(n log(n))
template <class T> void Graph<T>::printData(int index) {
  std::vector<std::pair<long long int, std::string>> stringIn;
  std::cout << "Listas de direcciones accesadas por: " << nodesInfo[index]
            << ": ";
  std::cout << std::endl;
  NodeLinkedList<std::pair<long long int, long long int>> *ptr =
      adjList[index].getHead();
  while (ptr != nullptr) {
    std::pair<long long int, int> par = ptr->data;
    int nodeV = par.first;
    long long int weight = par.second;
    std::string nodeData = nodesInfo[nodeV];
    // std::cout << nodeData;
    stringIn.push_back(std::make_pair(stringIpToInt(nodeData), nodeData));
    ptr = ptr->next;
    // std::cout << std::endl;
  }
  int n = stringIn.size();
  std::sort(stringIn.begin(), stringIn.end(), [](const auto &a, const auto &b) {
    return a.first > b.first; // Comparación de las IP de origen
  });
  for (int i = 0; i < n; i++) {
    std::cout << stringIn[i].second << std::endl;
  }
  std::cout << std::endl;
}
...
template <class T>
void Graph<T>::calculateInDegrees(const std::string &filename) {
  std::ofstream outFile(filename);
  if (!outFile.is_open()) {
    std::cerr << "Error al abrir el archivo: " << filename << std::endl;
    return;
  ...
  // Crear un vector para almacenar los in-degrees
  std::vector<int> inDegrees(numNodes, 0);

  // Recorrer la lista de adyacencia y contar los in-degrees
  for (int nodeU = 0; nodeU < numNodes; ++nodeU) {
    NodeLinkedList<std::pair<long long int, long long int>> *ptr =
        adjList[nodeU].getHead();
    while (ptr != nullptr) {
  ...
  // Escribir los in-degrees en el archivo
  outFile << "vertex,in-degree\n";
  for (int nodeU = 0; nodeU < numNodes; ++nodeU) {
    outFile << nodeU << "," << inDegrees[nodeU] << "\n";
  }

  outFile.close();
}
template <class T> void Graph<T>::displayDegrees(const std::string &filename) {
  std::ofstream outFile(filename);
  if (!outFile.is_open()) {
    std::cerr << "Error al abrir el archivo: " << filename << std::endl;
    return;
  ...
  std::vector<int> inDegrees(numNodes, 0);
  std::vector<int> outDegrees(numNodes, 0);

  // Calcular los grados de salida y los grados de entrada
  for (int nodeU = 0; nodeU < numNodes; ++nodeU) {
    NodeLinkedList<std::pair<long long int, long long int>> *ptr =
        adjList[nodeU].getHead();
    while (ptr != nullptr) {
      int nodeV = ptr->data.first;
      ++outDegrees[nodeU]; // Incrementar el grado de salida del nodoU
      ++inDegrees[nodeV];  // Incrementar el grado de entrada del nodoV
      ptr = ptr->next;
    }
  }

  // Escribir los grados de entrada y salida para cada nodo en el archivo
  outFile << "vertex,out-degree,in-degree\n";
  for (int nodeU = 0; nodeU < numNodes; ++nodeU) {
    outFile << nodesInfo[nodeU] << "," << outDegrees[nodeU] << ","
            << inDegrees[nodeU] << "\n";
  }
...
//O(1)
template <class T> int Graph<T>::getNumNodes() { return numNodes; }
